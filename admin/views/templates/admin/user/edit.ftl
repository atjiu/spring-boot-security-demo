	...
	<div class="row">
		<div class="col-md-8">
			<div class="box box-primary">
				...
				<!-- /.box-header -->
				<!-- form start -->
				<form role="form" id="form" action="/admin/user/edit" method="post">
					<input type="hidden" name="${_csrf.parameterName!}" value="${_csrf.token!}"/>
					<input type="hidden" value="${user.id?c}" name="id"/>
					<div class="box-body">
						<div class="form-group">
							<label for="username">用户名</label>
							<input type="text" class="form-control" readonly value="${user.username!}" id="username" placeholder="用户名">
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div>
								<#list roles as role>
									<input <#if role.id == user.role.id>checked</#if> type="radio" id="role_${role.id?c}" value="${role.id?c}" name="roleId"/>&nbsp;
