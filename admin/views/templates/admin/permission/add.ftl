				...
				<form role="form" id="form" action="/admin/permission/add" method="post">
					...
					<div class="box-body">
						<div class="form-group">
							...
							<select name="pid" id="pid" class="form-control">
								...
								<#list permissions as permission>
									<option value="${permission.id?c}" <#if pid?? && pid == permission.id>selected</#if>>${permission.description!}</option>
								</#list>
							</select>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" id="name" name="name" placeholder="角色名, 不能跟已存在的权限名重复, 如: user:list">
						</div>
						<div class="form-group">
